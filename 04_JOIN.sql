...
-- 작석 방법은 크게 ANSI 구문과 오라클 구문으로 나뉘고
-- ANSI에서 USING(같을때) 과 ON(다를때)을 쓰는 방법으로 나뉜다.
...
-- 테이블의 컬럼수를 기준으로 JOIN
--> 왼편에 작성된 테이블의 모든행이 결과에 포홤되어야 한다.
-- (JOIN이 안되는 행도 결과 포함)
...
-- 조인되는 테이블의 각 행들이 모두 매핑된 데이터기 검색되는 방법 (곱집합)
...
WHERE DEPT_CODE = DEPT_ID -- 여기서 조인
...
SELECT EMP_ID 사번, EMP_NAME 이름, JOB_NAME 직급명, DEPT_TITLE,LOCAL_NAME,SALARY
FROM EMPLOYEE E
JOIN JOB J ON(j.JOB_CODE = E.JOB_CODE) --  컬럼 이름이 같아서 J,E 등 구분 짓어야함/ 아니면 USING 사용
...
JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID )
JOIN JOB USING (JOB_CODE)
WHERE EMP_NAME LIKE '전%' - 성
AND SUBSTR(EMP_NO,8,1) ='2'  -- 여자
AND SUBSTR(EMP_NO,1,1) ='7' ; -- 70년대 생
--EMP_NO LIKE '7% -- 이것도 사용 가능  
...
JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID )
JOIN JOB USING (JOB_CODE)
WHERE DEPT_TITLE LIKE '해외영업1부' OR DEPT_TITLE LIKE '해외영업2부'
ORDER BY 부서명 ;
...
WHERE NATIONAL_NAME LIKE '한국'OR NATIONAL_NAME LIKE '일본';
