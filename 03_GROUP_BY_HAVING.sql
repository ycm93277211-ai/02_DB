...
-- EMPLOYEE 테이블에서 (FROM)
-- 성별(남/여)과 각 성별 별 인원수(GROUP BY), 급여 합을 (SELECT)
...
SUM(SALARY)"급여 합"
FROM EMPLOYEE
GROUP BY DECODE(SUBSTR(EMP_NO,8,1),'1','남','2','여') -- 별칭 사용 불가능 (SELECT 해석 전)
ORDER BY "인원 수";-- 별칭 사용 가능(SELECT 해석 후)
...
SELECT DEPT_CODE ,ROUND(AVG(SALARY))평균급여,COUNT(*)"인원 수"
FROM EMPLOYEE
WHERE DEPT_CODE IN('D5','D6') -- 부서코득 D5,D6인
GROUP BY DEPT_CODE;
-- 부서의 (WHERE절에서 구한 부서코드가 D5,D6인 데이터를 그룹으로 묶음)

-- EMPLOYEE 테이블에서 (FROM절 )
-- 2000년도 이후 입사자들의  (WHERE GROUP BY)
-- 직급별 급여합을 조회
-- 직급코드, 급여합 조회 SELECT
SELECT *
FROM EMPLOYEE

SELECT JOB_CODE ,SUM(SALARY) 합  
FROM EMPLOYEE
WHERE
HIRE_DATE >= TO_DATE('2000-01-01')
--방법 2 EXTRACT(YEAR FROM HIRE_DATE) >=2000
GROUP BY JOB_CODE;
...
-- EMPLOYEE 테이블에서
-- 부서별로 같은 직급인 사원의 인원수를 조회  (부서로 묶고 가은 직급도 묶고)
-- 부서코드 오름차순, 직급코드 내림차순으로 정렬
-- 부서코드, 직급코드, 인원수 SELECT
...
-- HAVING 절 : 그룹함수로 구해 올 그룹(GROUP BY)에 대한 조건을 설정할 때 사용
...
FROM EMPLOYEE 
GROUP BY DEPT_CODE -- 여기서 D1,D2 이런 식으로 그룹화 함
HAVING AVG(SALARY) >= 3000000 -- 그룹 중 급여 평균이 3백만원 이상인 그룹만 조회
ORDER BY DEPT_CODE ;
...
-- POLLUP : GROUP BY 절에서 가장먼저 작성된 컬럼의 중간집계를 처리하는 함수
