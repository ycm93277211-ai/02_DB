...
GRANT CREATE VIEW TO kh_ycm;


-- VIWE 생성 (kh_XXX 계정으로 접속 후 진행)
CREATE VIEW  V_EMP AS SELECT *FROM EMPLOYEE;
--ORA-01031: 권한이 불충분합니다

SELECT * FROM V_EMP;

-- 사번, 이름, 부서명, 직급명 조회하기 위한 VIEW 생성(OR REPLACE 덮어쓰기)
...
ORDER BY 사번;
--ORA-00955: 기존의 객체가 이름을 사용하고 있습니다 해결방법 OR REPLACE 사용(기존 객체 덮어쓰기)
...
VALUES('D0','L2');
--> 가상테이블 VIEW에 INSERT 가 성공할것을 확인 함.
SELECT *FROM V_DCOPY2;
...
-- 가능하면 의도되지 않는 NULL은 존재하지 않게 해야함.

-- 무결성 : 데이터베이스에 데이터를 정확하고 일관되게 유지하기 위한 중요한 개념
-- 데이터의 정확성, 일관성, 신뢰성을 보장함



-- WITH READ ONLY 옵션 사용하기
-- VIEW를 이용해서 DML 를 막기 위해 사용
CREATE OR REPLACE VIEW V_DCOPY2
AS SELECT DEPT_ID,LOCATION_ID FROM DEPT_COPY2
...
-- SEQ_TEST_NO의 MAXVALUSE 값을 200으로 수정
 ...
 * 즉, 인덱스의 핵심 역할은 컬럼값 -> 해당 행의 ROWID를 통해서 빠르게 찾아내는 것!
 * 
 * */
 ...
 * UNIQUENESS : UNIQUE 여부 (UNIQUE/NONUNUQUE)
...
-- TB_IDX_TEST 테이블에 
-- 샘플 데이터 100만개 삽입 (PL/SQ 사용)
-- 조건문(IF/CASE), (반복문LOOP,WHILE), 변수 선언 등.... 가능
