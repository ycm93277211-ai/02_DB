...
CREATE USER ycm_sample IDENTIFIED BY sample1234;
...
-- [권한 부여 작성법]
-- GRANT 권한,권한,...TO 사용자명;
GRANT CREATE SESSION TO ycm_sample;
...
-- CREATE TABLE: 테이블 생성 권한이 없음
-- + 데이터를 저장할 수 있는 공간(TABLESPACE) 할당

--6. (SYS) 테이블 생성 권한 부여 + TABLESPACE 할당
GRANT CREATE TABLE TO ycm_sample;

ALTER USER ycm_sample DEFAULT TABLESPACE 
SYSTEM QUOTA UNLIMITED ON SYSTEM;
-- SYSTEM 테이블스페이스를 사용자 
-- ycm_sample의 저장소로 무제한 사용가능하게끔 하는 명령어
...
GRANT CONNECT,RESOURCE TO ycm_sample;

-- CONNECT : DB 접속 권련 권한을 묶어둔 ROLE
...
GRANT SELECT ON EMPLOYEE TO ycm_sample;

-- 3. (sample) 다시 EMPLOYEE 조회
SELECT * FROM kh_ycm.EMPLOYEE;
-- 가능!!!1

-- 4.(kh) smaple 계정에 부여했던 EMPLOYEE 테이블 조회 권한 회수(REVOKE)
REVOKE SELECT ON EMPLOYEE FROM ycm_sample;


-- 5. (sample) 다시 EMPLOYEE 조회
SELECT * FROM kh_ycm.EMPLOYEE;
-- 조회 불가능!!!(권한 회수 완료!)
