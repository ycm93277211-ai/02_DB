...
-- LENGTH(컬럼명 OR 문자열 ): 길이 반환

-- EMPLOYEE 테이블에서 모든 사원의
-- EMAIL,EMAIL의 길이를 조회
SELECT EMAIL 이메일,LENGTH(EMAIL)"이메일 길이"
FROM EMPLOYEE ;

------------------------------------------------------
--[]은 옵션
-- INSTR(컬럼명 OR 문자열 ,'찾을문자열' [, 찾기 시작할 위치] [,순번])
-- 지정한 위치부터 지정한 순번째로 감색되는 문자의 위치를 반환

-- 문자열을 앞에서 부터 검색하여 첫번째 B 위치 조회
-- AABAACAABBAA - > 0부터가 아닌 1부터
...
-- SUBSTR(컬럼명 OR 문자열, 잘라내기 시작할 위치 [, 잘라낸 길이])
-- 컬럼이나 문자열에서 지정한 위치부터 지정된 길이만큼 문자열을 잘라내어 반환
--> 잘라낸 길이 생략 시 끝까지 잘라냄

--EMPLOYEE 테이블에서 사원명, 이메일, 이메일 중 아이디만 조회
--sun_di@or.kr -> sun_di
...
------------------------------------------------------------
--[옵션]이 앞에 있음
-- TRIM([문자열 OR 컬럼명 FROM]컬럼명 OR 문자열)
...
-- MOD(숫자 OR 컬럼명,숫자 OR 컬럼명): 나머지 값 반환
-- EMPLOYEE 테이블에서 사원의 월급을 100만원으로 나눴을때 나머지값 조회
...
--CEIL(숫자 OR 컬럼명): 올림
--FLOOR(숫자 OR 컬럼명): 내림
--> 둘다 소수점 첫쨰 자리에서 올림/내림 처리

SELECT CEIL(123.1), FLOOR(123.9) FROM DUAL;

-- TRUNC(숫자 OR 컬럼명 [, 위치]): 특정 위치 아래를 절삭
...
--SYSTIMESTAMP: SYSDTAE + MS 단위 추가 (UTC 정보)
SELECT SYSTIMESTAMP FROM DUAL;ALTER 
...
SELECT TO_CHAR(100000,'L9,999,999') || '원' FROM DUAL;
...
-- 2025년 10월 20일 (월)
--SELECT TO_CHAR(SYSDATE,'YYYY년/MM월/DD일 (DY)')-> 오류
...
--1) 주민번호에서 - 앞글자까지 추출 즉 짤라내기 해야함
SELECT EMP_NAME,
SUBSTR(EMP_NO,1,INSTR(EMP_NO,'-')-1) "생년월일"
FROM EMPLOYEE;
...
SELECT EMP_NAME,
TO_CHAR(TO_DATE(SUBSTR(EMP_NO,1,INSTR(EMP_NO,'-')-1), 'YYMMDD'), 'YY"년"MM"월"DD"일"') 
"생년월일"
FROM EMPLOYEE;
...
-- EMPLOYEE 테이블에서 보너스르 받으면 '0' ,안받으면 'X' 조회
...
-- 가장 큰 급여 조회
SELECT MAX(SALARY)FROM EMPLOYEE; -- 소브쿼리 + 그룹함수
